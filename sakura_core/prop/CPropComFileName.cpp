...
*/
#include "StdAfx.h"
#include "prop/CDlgConfigChildFileName.h"
...
HWND CDlgConfigChildFileName::DoModeless( HINSTANCE hInstance, HWND hwndParent, SDlgConfigArg* pDlgConfigArg, int nTypeIndex )
{
	m_nCurrentTypeIndex = nTypeIndex;
	m_pDlgConfigArg  = pDlgConfigArg;

	return CDialog::DoModeless( hInstance, hwndParent, IDD_PROP_FNAME, 0, SW_SHOW );
}


BOOL CDlgConfigChildFileName::OnInitDialog( HWND hwndDlg, WPARAM wParam, LPARAM lParam )
{
	RECT		rc;
	LV_COLUMN	col;
...
BOOL CDlgConfigChildFileName::OnNotify( WPARAM wParam, LPARAM lParam )
{
...
BOOL CDlgConfigChildFileName::OnBnClicked( int wID )
{
	HWND hwndDlg = GetHwnd();
	HWND hListView = GetDlgItem( hwndDlg, IDC_LIST_FNAME );
	int nIndex = ListView_GetNextItem( hListView, -1, LVNI_SELECTED );
...
*/
void CDlgConfigChildFileName::SetData()
{
	HWND hwndDlg = GetHwnd();
	int nIndex;
	int i;
...
int CDlgConfigChildFileName::GetData()
{
	HWND hwndDlg = GetHwnd();
...
int CDlgConfigChildFileName::SetListViewItem_FILENAME( HWND hListView, int nIndex, LPTSTR szFrom, LPTSTR szTo, bool bInsMode )
{
	LV_ITEM	Item;
	int nCount;
...
void CDlgConfigChildFileName::GetListViewItem_FILENAME( HWND hListView, int nIndex, LPTSTR szFrom, LPTSTR szTo )
{
...
int CDlgConfigChildFileName::MoveListViewItem_FILENAME( HWND hListView, int nIndex, int nIndex2 )
{
	TCHAR szFrom[_MAX_PATH];
	TCHAR szTo[_MAX_PATH];
...
LPVOID CDlgConfigChildFileName::GetHelpIdTable()
{
