...
void CDocOutline::MakeTopicList_html(CFuncInfoArr* pcFuncInfoArr, bool bXml)
{
	...
	bool			bCDATA = false;
	bool			bRawString = false; // script,style要素用
	...
	*/
	...
	enum ELabelType {						//	列挙体：ラベルの種別
		...
		LT_BLOCK,		LT_PARAGRAPH,	LT_HEADING,		LT_IN_RAW
	};
	...
	*/
	...
	for(nLineCount=CLogicInt(0);nLineCount<m_pcDocRef->m_cDocLineMgr.GetLineCount();nLineCount++)
	{
		...
		for(i=0;i<nLineLen-1;i++)
		{
			...
			szTitle[j] = '\0';
			if (bRawString) {
				// 2014.12.26 Moca </script </style までスキップ
				// HTML5ではネストされた場合ネストを許すがここでは未対応
				if (!bEndTag) {
					continue;
				}
				if (wcsicmp(szTag, szTitle)) {
					// szTag != szTtile
					continue;
				}
			}
...
//				}
				...
				if( !wcscmp(szTag,L"script") || !wcscmp(szTag,L"style") ){
					bRawString = true;
				}
			}
