...
void CConvertMediator::ConvMemory( CNativeW* pCMemory, EFunctionCode nFuncCode, CKetaXInt nTabWidth, int nStartColumn )
{
	...
	//文字種変換、整形
	case F_TABTOSPACE:				CConvert_TabToSpace((Int)nTabWidth, nStartColumn).CallConvert(pCMemory);break;	// TAB→空白
	case F_SPACETOTAB:				CConvert_SpaceToTab((Int)nTabWidth, nStartColumn).CallConvert(pCMemory);break;	// 空白→TAB
