...
bool CFuncLookup::Funccode2Name( int funccode, WCHAR* ptr, int bufsize ) const
{
	...
	if( F_USERMACRO_0 <= funccode && funccode < F_USERMACRO_0 + MAX_CUSTMACRO ){
		...
		if( L'\0' == pName[0] ){
			pName = LSW(STR_MARKER_PRESET_NAME1 + nSetIndex);
		}
		auto_snprintf_s(ptr, bufsize, LSW(STR_MENU_COLORMARKER_SET), nSetIndex + 1, pName);
		return true;
	}
...
const WCHAR* CFuncLookup::Custmenu2Name( int index, WCHAR buf[], int bufSize ) const
{
	...
	if( index == 0 ){
		...
		wcscpyn( buf, LSW( STR_CUSTMENU_COLORMARKER ), bufSize );
		return buf;
	}
